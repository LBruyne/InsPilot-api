prompt_deep_divergence_vision = (
    '上面是一张关于{task}的草图，它的功能是：{input}'
    '请结合草图和功能，猜猜它是什么产品。'
    '方案名需要满足以下要求：'
    '（1）5-8个字。'
    '输出要求：只需要输出方案名，不要有序号、引号或其他任何文字和符号！'
    '输出示例：便携折叠跑步机'
)
...
prompt_deep_divergence_4 = (
    ...
    '输出要求：只需要输出产品功能表述的内容，每个内容之间用#隔开，不要有序号或任何其他的文字、符号、数字，包括对于提示词的解释内容！'
    '设计方案：<{input}>'
)